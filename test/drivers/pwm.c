 ...
 *
 * ---------------------------------------------------------------------*/
void pwm_polarity(struct pwmInfo *pwm)
{
	timer_set_polarity(pwm->ctlr, pwm->polarity);
}	

/* -- pwm_period ---------------------------------------------------------
 *
 * ---------------------------------------------------------------------*/
void pwm_period(struct pwmInfo *pwm)
{
	/* set timer period */
	timer_set_period(pwm->ctlr, pwm->period);
}

/* -- pwm_duty_cycle -----------------------------------------------
 *
 * ---------------------------------------------------------------------*/
void pwm_duty_cycle(struct pwmInfo *pwm)
{
	...
	else
	{
		...
		match = (0xFFFFFFFF - tldr)*pwm->duty_cycle/100;
		match += tldr;
