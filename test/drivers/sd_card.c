 ...
 *
 * ---------------------------------------------------------------------*/
unsigned int sdcard_write(struct mmcsdInfo *mmcsd, void *ptr, 
											unsigned int block, unsigned int nblks)
{
   ...
   */
	...
	mmcsd_set_blklen(mmcsd->ctlr, 512);
	...
	if(sdcard_send_cmd(mmcsd, &cmd) == 0)
	{
		PRINT("CMD FAILED\n\r");
		return 0;
	}
	...
	/* write data */
	if(sdcard_write_data(mmcsd, ptr, nblks) == 0)
	{
		PRINT("DATA READ FAILED\n\r");
		return 0;
	}

	if(mmcsd_get_tran_status(mmcsd->ctlr, 0) == 0)
	{
		PRINT("TRANS FAILED\n\r");
		return 0;
	}
 ...
 *
 * ---------------------------------------------------------------------*/
unsigned int sdcard_read(struct mmcsdInfo *mmcsd, void *ptr, 
													unsigned int block, unsigned int nblks)
{
	struct cardInfo *card = mmcsd->card;
	unsigned int address;
   ...
   */
	...
	/* read data */
	if(sdcard_read_data(mmcsd, ptr, nblks) == 0)
		return 0;
 ...
 *
 * ---------------------------------------------------------------------*/
unsigned int sdcard_read_data(struct mmcsdInfo *mmcsd, void *ptr, 
																	unsigned int nblks)
{
	unsigned int i;
	...
	for(i=nblks;i!=0;i--)
	{
		if(sdcard_read_blk(mmcsd,512,data) == 0)
			return 0;
		data += 128; /* 512/4 */
	}
 ...
 *
 * ---------------------------------------------------------------------*/
unsigned int sdcard_write_data(struct mmcsdInfo *mmcsd, void *ptr, 
												unsigned int nblks)
{
	unsigned int i;
	unsigned int *data = (unsigned int *)ptr;
	...
	for(i=nblks;i!=0;i--)
	{
		if(sdcard_write_blk(mmcsd, 512, data) == 0)
			return 0;
		data +=  128; /* 512/4 */
	}
 ...
 *
 * ---------------------------------------------------------------------*/
unsigned int sdcard_write_blk(struct mmcsdInfo *mmcsd, unsigned int blklen,
                              unsigned int *data)
{
	unsigned int i;
	...
	while(!mmcsd_write_complete(mmcsd->ctlr));
	
	mmcsd_clear_status(mmcsd->ctlr, MMC_STAT_BWR);

	/* write data to data buffer */
	for(i=blklen;i!=0;i--)
	{
		mmcsd_write_data(mmcsd->ctlr,*data++);
	}
 ...
 *
 * ---------------------------------------------------------------------*/
unsigned int sdcard_stop(struct mmcsdInfo *mmcsd)
{
	struct sdCmd cmd;
	...
	return sdcard_send_cmd(mmcsd, &cmd);
}

/* -- sdcard_set_buswidth ------------------------------------------------
 *
 * ---------------------------------------------------------------------*/
unsigned int sdcard_set_buswidth(struct mmcsdInfo *mmcsd)
{
	...
	if((mmcsd->buswidth & SD_BUSWIDTH_4BIT) && 
	...
	if(sdcard_send_acmd(mmcsd, &acmd) == 0)
		return 0;
	
	/* configure controller to operate with same buswidth */
	mmcsd_bus_width(mmcsd->ctlr, (0x1 << acmd.arg));

	/* update info struct */
	mmcsd->buswidth = 0x1 << acmd.arg;
 ...
 *
 * ---------------------------------------------------------------------*/
unsigned int sdcard_set_bus_freq(struct mmcsdInfo *mmcsd)
{
	struct cardInfo *card = mmcsd->card;
	struct sdCmd cmd;
	/* unsigned int freq; */
		...
		{
			mmcsd->busfreq = 48000000;	/* max frequency we're capable of */
		}
		else
		{
			mmcsd->busfreq = 25000000;
		}

		/* set mmcsd bus frequency */
		if(mmcsd_set_bus_freq(mmcsd->ctlr,mmcsd->busfreq) != 0)
			return 0;
 ...
 *
 * ---------------------------------------------------------------------*/
int sdcard_init(struct mmcsdInfo *mmcsd)
{
	unsigned int i;
	...
	/* Enable Interrupt flags.  Note we are not using interrupts but we 
 	 * polling these flags.
   */
	sdcard_en_stat_flags(mmcsd);

	/* CMD0 - Reset card */
	PRINT("Card Reset\n\r");
	retval = sdcard_reset(mmcsd);
	...
	/* check card type */
	PRINT("Type Check\n\r");
	retval = sdcard_check_type(mmcsd);	
	retval = 1;

	if(retval == 1)
	{
		/* set card type */
		mmcsd->card->cardType = MMCSD_CARD_SD;
		...
		retval = sdcard_send_cmd(mmcsd, &cmd);
		if(retval == 0)
			return 0;
		...
		mmcsd_set_blklen(mmcsd->ctlr, 8);

		if(sdcard_send_acmd(mmcsd, &cmd) == 0)
			return 0;
	
	
		/* Get data from buffer */			
		if(sdcard_read_blk(mmcsd, 8, (unsigned int *)dataBuffer) == 0)
		{
			PRINT("NO Data\n\r");
			return 0;
		}
 ...
 *
 * ---------------------------------------------------------------------*/
unsigned int sdcard_read_blk(struct mmcsdInfo *mmcsd, unsigned int blklen,
															unsigned int *data)
{
	unsigned int i;
	...
	/* check to see if data is ready */
	while(!mmcsd_read_complete(mmcsd->ctlr));
	mmcsd_clear_status(mmcsd->ctlr,MMC_STAT_BRR);

	/* copy mmcsd data buffer into local buffer */
 	/* NOTE: blklen/4 to handle 32bit register access */
	for(i=0;i<blklen;i++)
	{
		*data++ = mmcsd_read_data(mmcsd->ctlr);
	}
 ...
 * 
 * ---------------------------------------------------------------------*/
unsigned int sdcard_check_type(struct mmcsdInfo *mmcsd)
{
	struct sdCmd cmd;
	unsigned int status;
	...
	status = sdcard_send_acmd(mmcsd, &cmd);
	return status;	
}
 ...
 *
 * ---------------------------------------------------------------------*/
unsigned int sdcard_send_acmd(struct mmcsdInfo *mmcsd, struct sdCmd *cmd)
{
	...
	cmdapp.flags = 0;
	cmdapp.arg = mmcsd->card->rca << 16;
	
	if(sdcard_send_cmd(mmcsd, &cmdapp) == 0)
		return 0;

	status = sdcard_send_cmd(mmcsd, cmd);
 ...
 * 
 * ---------------------------------------------------------------------*/
int sdcard_reset(struct mmcsdInfo *mmcsd)
{
	struct sdCmd cmd;
	unsigned int status;
 ...
 *
 * ---------------------------------------------------------------------*/
int sdcard_send_cmd(struct mmcsdInfo *mmcsd, struct sdCmd *cmd)
{
	...
	if(dataPresent)
	{
		...
		mmcsd_clear_status(mmcsd->ctlr, MMC_STAT_TC);
		/* set data transfer timeout */
		mmcsd_data_timeout(mmcsd->ctlr, MMC_DATA_TIMEOUT(27));
	}
	...
	mmcsd_send_cmd(mmcsd->ctlr, mmcCmd, cmd->arg, 
										(void *)dataPresent, nblks, 0);

	/* See if command went through */
	status = mmcsd_get_cmd_status(mmcsd->ctlr);


	/* Get command response */
	if(status == 1)
	{
		mmcsd_get_rsp(mmcsd->ctlr, cmd->rsp);
	}
 ...
 * ---------------------------------------------------------------------*/
void sdcard_en_stat_flags(struct mmcsdInfo *mmcsd)
{
	unsigned int flags;
	...
	mmcsd_en_status_flags(mmcsd->ctlr, flags);
}
